...
public class Education extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		bk = (Button) findViewById(R.id.btnEducationBack);

		dbAdapter = new DatabaseAdapter(this);
		dbAdapter.open();

		try {
			listview = (ListView) findViewById(R.id.list_all_education);
			if (idclick1 != 0) {
				ide = idclick1;
				data = dbAdapter.getAllEducationData(idclick1);
			...
			listview.setOnItemLongClickListener(new OnItemLongClickListener() {
				...
				public boolean onItemLongClick(AdapterView<?> arg0, View arg1,
					...
					final String itemAt = listview.getItemAtPosition(arg2)
							.toString();
					build = new AlertDialog.Builder(Education.this);
	...
	protected void onResume() {
		...
		data = dbAdapter.getAllEducationData(ide);
		view = new ArrayList<String>();
		for (int i = 2; i <= data.size(); i = i + 3) {
			view.add(data.get(i));
		}
